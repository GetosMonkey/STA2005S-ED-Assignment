...
We created the design matrix in accordance to a **Nested Latin Square design**, testing all 16 heat $\times$ light combinations in each block where each block represented a greenhouse-season combination and is created from a  2$\times$2 Latin square design with greenhouse ( A, B) as columns and season (1, 2) as rows. 
...
design_csv <- final_design[c("season", "greenhouse", "light", "heat",
                             "variety", "side", "plot")]
```
...
kable(coef_table, caption = "Model Coefficients Summary", digits = 3)
```

### Model Interpretation:

We fitted a linear model to analyze the experimental design, using **Season × Greenhouse** (`block`) to control for background variation. The model tested the main effects of **heat**, **light**, **variety**, and **side**, as well as all two-way interactions involving the primary treatment factors (`heat:light`, `heat:variety`, `light:variety`).

Initially, a more complex model was tested, which included the full three-way interaction (heat:light:variety). This interaction was found to be non-significant and was subsequently removed to simplify the analysis. The model was also simplified by treating side as a main effect rather than nesting it within block (block:side). While the side effect was found to be highly significant (p < 0.001) and thus an important nuisance factor to control for, the nested version (block:side) caused over-parameterization due to the missing data. The final, simplified model (obs ~ block + side + heat*light + variety + heat:variety + light:variety) was adopted as it robustly controls for all key factors and allows for stable estimation of the treatment effects.
...
The ANOVA results showed highly significant main effects of heat, light, and side, but insignificant interactions between heat and variety as well as between light and variety (p>0.05). These effects indicate that increasing temperature and light intensity both improved outcomes. The blocking factors (Season × Greenhouse) also explained a significant portion of variation (p = 0.003 < 0.05), confirming that the blocking was effective in controlling background differences.
...
1) **Heat × Light:** Quality increases with Heat from level 1 to 3 for all Light levels, with a slight plateau or dip at Heat 4. The lines are roughly parallel, indicating minimal interaction. This shows that Heat and Light act mostly additively, with each factor independently improving chilli quality. Differences between Light levels remain consistent across Heat levels, highlighting strong main effects.

2) **Heat × Variety:** Both varieties (Redhot and Furious) improve with increasing Heat. The lines are mostly parallel, with only a small divergence at Heat 4, where Redhot slightly outperforms Furious. This suggests that the Heat effect is consistent across varieties, with minimal Heat × Variety interaction.

3) **Light × Variety:** Quality rises sharply with Light from 1 to 3 and flattens slightly at Light 4. Lines for both varieties nearly overlap, indicating that Light affects both varieties similarly. This implies minimal Light × Variety interaction, confirming Light as a strong main effect rather than an interactive one.

Overall, the plots show that Heat and Light are the dominant factors, with both varieties responding similarly. Optimal quality is generally achieved around Heat 3–4 combined with Light 3–4.
...
We simplified the model by treating side as a main effect. This avoids the over-parameterization of prior models and yields stable, interpretable estimates for heat, light, and variety. This approach is justified as it appropriately controls for side as a known nuisance variable.
...
This means that after accounting for all other factors (like blocking, side, and the significant interaction identified in Q2), neither variety provides a statistically reliable advantage overall. The important difference is not the main effect, but rather the interaction—how the varieties respond differently to specific heat levels.
...
cld_hl <- cld(emm_hl, adjust = "tukey", Letters = letters)
kable(cld_hl, 
      caption = "CLD for Heat × Light Interactions", 
      digits = 2)
plot(emm_hl, comparisons = TRUE)

```
This display clearly shows that the top two combinations, (Heat 3, Light 4) and (Heat 4, Light 4), both share the letter 'g'. This confirms our earlier finding that while (H3, L4) has the highest mean, it is not statistically superior to (H4, L4).

---

# 🌞 Future Reccomendations:

**Primary Recommendation**: Based on this experiment, the setting of Heat 3 × Light 4 produced the highest mean quality (55.79).
...
1. Heat 4 × Light 4: This is the second-best option, achieving a mean quality (54.12) only   1.67 units below the maximum. This setting may be a safer general recommendation as it did not show a negative interaction with either variety.

2. Heat 3 × Light 3: This setting is a larger step down, with a mean quality (52.85) that is 2.94 units below the maximum.
...
design_csv <- final_design[c("season", 
                             "greenhouse", "light", 
                             "heat", "variety", "side", "plot")]
...
for (i in 1:nrow(all_main_blocks)) {
  ...
  side_variety_combos <- sort(unique(
    paste(current_block_data$side, 
  ...
  for (sv in side_variety_combos) {
    ...
    subset_data <- current_block_data[
      paste(current_block_data$side,
      current_block_data$variety) == sv, ]
    cat("    > Heat levels present: ", paste(
      sort(unique(subset_data$heat)), collapse=", "), "\n")
    cat("    > Light levels present:", paste(
      sort(unique(subset_data$light)), collapse=", "), "\n")
  }
  cat("\n")
}
...
# Fit model ------------------------------------------------------------------
fit <- lm(obs ~ block + side + heat*light + 
            variety + heat:variety + light:variety, data = dat)
...
# Create 'block' as interaction of season × greenhouse
dat_complete$block <- interaction(dat_complete$season, 
                                  dat_complete$greenhouse, sep = ".")
...
fit <- lm(obs ~ block + side + heat*light + 
            variety + heat:variety + light:variety, data = dat_complete)
...
# -------------------------------------------------------------------
#Anova(fit, type = 2)  
# look at heat:variety, light:variety, and heat:light:variety rows
...
# Remove problematic rows manually
decision_df <- decision_df[!is.na(decision_df$emmean) 
                           & !is.na(decision_df$SE), ]
